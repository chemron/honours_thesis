...
A (STEREO-A)\footnote{STEREO consists of
two spacecraft, STEREO ahead (A) and STEREO behind (B), however, contact with
STEREO-B was lost in 2016, and so the images used were taken by STEREO-A.}, to
generate magnetograms from STEREO-A data. The orbit of STEREO is such that it
moves about the Sun relative to Earth, giving it some view of the farside
throughout it's mission, hence providing farside magnetograms at some points in
it's orbit. However STEREO-A currently only has a partial view of the solar
farside, a view that will only decrease until it passes the Earth in 2023.




\citet{Kim2019} used a deep learning model known as a conditional Generative
Adversarial Network (cGAN) to learn a mapping between extreme ultraviolet (EUV)
images and magnetograms. This was than applied to EUV images taken by NASA's
Solar Terrestrial Relations Observatory A (STEREO-A)\footnote{STEREO consists of
two spacecraft, STEREO ahead (A) and STEREO behind (B), however, contact with
STEREO-B was lost in 2016, and so the images used were taken by STEREO-A.}, to
generate magnetograms from STEREO-A data. The orbit of STEREO is such that it
moves about the Sun relative to Earth, giving it some view of the farside
throughout it's mission, hence providing farside magnetograms at some points in
it's orbit. However STEREO-A currently only has a partial view of the solar
farside, a view that will only decrease until it passes the Earth in 2023. By
then, STEREO-A will be more than 16 years into its planned 2-year mission and
may have lost contact with the Earth. Furthermore \citet{Kim2019} used a
saturation point of 100 Gauss when generating these magnetograms. As many active
regions have magnetic fields stronger than 1000 G, this severely limits the
ability to predict extreme events.\\


An alternative way to image the farside of the sun is given by farside
helioseismic holography \citep{lindsey_seismic_2000}. This is a helioseismic
technique that maps the farside by timing accoustic waves, known as p-modes, as
they travel from the nearside to the farside and back again. Disturbances on the
farside are then observable by the variation in this travel time.
\citep{felipe_improved_2019} 



We employ a
method to generate synthetic magnetograms from farside seismic maps. Our method
consists of two steps. First, we train a cGAN to generate magnetograms from EUV
images using data from the Solar Dynamics Observatory (SDO). This cGAN is then
applied to EUV images from STEREO-A to create a data set of partially farside
synthetic magnetograms. Secondly, we use these synthetic magnetograms to train a
separate cGAN to generate magnetograms from farside seismic maps.
...
\begin{itemize}
  ...
  \item etc
  \item problem
  \item rotation of the sun
  \item what do we need
  \item furthermore provide boundary conditions (maybe)
  \item deep learning
  \item what has been done
  \item kim
  \item problem with stereo
  \item helioseismology
  \item farside helioseismic holography
  \item what we do
  \item signpost?
\end{itemize}
...
% TODO: probably just remove this part about kim



% To avoid these issues, we instead 
