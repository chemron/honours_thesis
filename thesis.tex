...
\end{align}
% TODO: should I include this? probably not
...
to be projected against the celestial sphere itself.  %TODO: check 0.3
...
Perhaps a bigger limitation however is the lack of information. If the
descriminator network does not have enough information (based on the EUV or
seismic data) to determine whether the magnetogram produced the generator
network is accurate, the generator will not learn to become more accurate. This
effectively leads to the generator `guessing' features without penalty, as long
as they appear realistic. For example, while both GANs learnt that a positive
magnetic field was typically associated with a negative magnetic field neither
was able to guess the polarity better than as by using Hale's law (see Section
\ref{sec:dynamo}). This is likely a consequence of the lack of infomation, as
the EUV and seismic data itself is unlikely contain sufficient information to
determine the polarity. It was thought that this information may have been
determinable from context, however this was not sufficient to consistently
determine the correct polarity. The lack of information was a greater problem
for the seismic GAN, which gave mixed results when attempting to determine the
existance of active regions and of the active regions it did predict, it was
unable to accurately determine the correct shape or polarity. It is possible
that after more training the cGAN's may have been able to learn more complex
relationships and some of the underlying phsyics, but this begins to cut in to
the available resources for training. As it was, fully training a cGAN required
roughly four days of computation time using an expensive GPU. \\
...
percentile pixel value. While this 
