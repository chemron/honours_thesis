...
Composite images of the farside phase map and the corresponding nearside
...
\begin{figure}[t]%
  ...
  \caption[]{
    ...
    \citep{Lindsey2011}.\\
    \subref{fig:phase_map} Images showing both a farside phase map created using
    ...
    nearside. The phase map is measured by the time perturbation, \(\tau\) caused by
...
phase maps every 12 hours by Stanford's Joint Science Operations
...
\begin{itemize}
  ...
  holography.
\end{itemize}
Solar Magnetograms - images of the magnetic field of the Sun's surface, are an
important tool for understanding the solar magnetic field and its evolution
\citep{Solanki_2006}. 
Currently, magnetograms are only available for the
nearside - the side of the Sun facing the Earth at any given moment, which are
taken with the Helioseismic and Magnetic Imager (HMI) aboard the Solar Dynamics
Observatory (SDO) satellite \citep{nasa_sdo}. Relative to Earth, the Sun rotates
with a period of approximately 28 days\footnote{Different latitudes rotate at
different rates and so this is just an average (see section
\ref{sec:internal_structure})} \citep{beck_comparison_2000}. Therefore a method
to image the farside magnetic field would be beneficial for not only modelling
the global magnetic field of the Sun, but also for predicting extreme space
weather events ahead of time. \par

There are currently nine active solar probes observing the Sun\footnote{These
are: Advanced Composition Explorer (ACE), Deep Space Climate Observatory
(DSCOVR), Hinode, Interface Region Imaging Spectrograph (IRIS), Parker Solar
Probe, Solar Dynamics Observatory (SDO), Solar and Heliospheric Observatory
mission (SOHO), Solar Orbiter, Solar-Terrestrial Relations Observatory A (STEREO
A), and Wind. See
https://www.nasa.gov/mission_pages/sunEarth/missions/index.html}. Of these nine
spacecraft, only three (Parker Solar Probe, Solar Orbiter and STEREO-A) are in
heliocentric orbits, and so only these satellites are capable of observing the
farside at any point in their orbit. Of these three, none are capable of taking
magnetograms. Therefore, an indirect measurement of the magnetic field must be
employed to reliably predict extreme space weather events .\par

\citet{Kim2019} trained a conditional Generative Adversarial Network (cGAN) (see
section \ref{sec:cgan}) to generate artificial magnetograms from Extreme
Ultraviolet (EUV) images. This was done by associating the EUV images from the
SDO's Atmospheric Imaging Assembly (AIA) with the SDO's HMI magnetograms. While
the SDO has a geocentric orbit (i.e. it orbits the Earth) and can only see the
solar nearside, the STEREO-A spacecraft is on a heliocentric orbit and observes
the solar farside at some points in its orbit. As such, \citet{Kim2019} were
able to generate artificial farside magnetograms based on EUV images from
STEREOs extreme ultraviolet Imager (EUVI). This method has since been improved
upon by \citep{Alshehhi2020} using a different discriminator model (see section
\ref{sec:gan}).\par

However, STEREO-A is currently on the nearside coming towards Earth and only has
a partial view  the solar farside (see figure %TODO:
). Furthermore, it will not pass Earth and begin
to return to the farside until August 2023\footnote{See:
https://stereo-ssc.nascom.nasa.gov/where.shtml}. The satellite launched in 2006
and had a planned mission duration of 2 years \citep{nasa_stereo}. It is
therefore unlikely that STEREO-A will be able to reach the farside once more before
a loss of contact. \par
\begin{itemize}
  \item 
\end{itemize}
...
\begin{itemize}
  \item we want to generate farside magnetograms from farside seismic maps
  \item 
...
\begin{itemize}
  ...
  \item as the global magnetic field flips every solar cycle (see Section
        \ref{sec:dynamo}), this gave consistent magnetograms across the
        data set.
