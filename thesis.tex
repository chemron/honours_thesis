...
\begin{itemize}
  ...
  weighting are shown in Figure \ref{fig:aia_no_degredation}.
  ...
  the STEREO data is shown in Figure \ref{fig:stereo_percentiles}. Clearly the image
  
  
  the images from
  these datasets.  The
  zero point for the AIA data seemed like a good overall zeropoint,
  ~3% of the AIA data was below zero
  Conversely, for the stereo data the same percentile of data had a value below
  725, i.e. a value of zero in a pixel from AIA was roughly equivalent to a
  value of 725 in a STEREO pixel Therefore STEREO data was normalised in the
  following steps:
  
  \item As can be seen in Figure \ref{fig:aia_percentiles}, pixel values above
  $\approx \SI[]{2e4}[]{DN}$ are clipped by the instrument. After
  accounting for the instrument degradation, this becomes the wavy line seen in Figure
  \ref{fig:aia_no_degredation}. To avoid the GAN learning any relation from this
  we must make the clipping uniform across the whole dataset. However since we
  need to directly relate the AIA images to the STEREO images, this also needs
  to match the clipping of STEREO. 
  
  The same procedure was applied to the STEREO data set, with the result
  after each step shown in Figure 
\end{itemize}
