...
\begin{enumerate}
  ...
  \citep{van_driel-gesztelyi_evolution_2015}. This emergence of new active
  regions gives the motivation for imaging the farside in the first place!
\end{enumerate}
...
position of STEREO-A at time $t_s$. It should be noted that $\theta(t_s)$ and
therefore $\theta(t_s)$ are negative while STEREO-A is `behind' the solar rotation, and
positive while STEREO-A is `ahead' of the solar rotation. We can therefore
calculate the equivalent farside time ($t_f$) for a given stereo time as follows
\begin{align}
  t_{farside} = t_{stereo} - \Delta t(t_s) \, .
\end{align}
