...
\begin{figure}[t]%
  ...
  range of data, the 25th to 100th percentiles were plotted on a log scale.
...
\begin{figure}[t]
  ...
  \caption{Comparison of the SDO and STEREO-A data after normalisation.}
...
evaluate the quality of its input.
  ...
  \begin{figure}
    ...
    \caption{
      ...
      in the downsampling path, apply convolution, batch normalisation and leaky
      reLU activation.% TODO need to explain these
      At each convolutional step, an additional 64 filters are used than on the
...
predicting extreme magnetic fields. One such metric could be the average
pixel-wise difference between the true and synthetic magnetograms. This is shown
in Figure \ref{fig:uv_loss} for a given batch at each training iteration. This
error is difficult to interpret however and does not take into account the relative
importance of pixels. For example, a pixel near the centre of a magnetogram will
subtend a smaller region of the solar surface than a pixel closer to the solar
limb. This effectively means that the absolute error between images will be
unevenly weighted towards the central pixels. A better metric for evaluation is
the unsigned magnetic flux, \(T_{\text{flux}}\) given by
\begin{align}
...
with the final image being an integration over ... hours, while the SDO images
are essentially instantaneous. %TODO: hours
...
Seismic-GAN. Despite these flaws, solar flares occurances had associated peaks
...
time along with a true SDO nearside magnetogram twelve days later.
...
method. Perhaps the most obvious limitation was the lack of any true
...
magnetograms themselves were not perfect, these provide an upper bound to the
...
This also means that the Seismic-GAN may learn `quirks' of the UV-GAN, as its
...
7\) days before directly facing the Earth due to the rotation of the Sun. He we
...
unsigned magnetic flux which peaks during times of intense solar activity.
...
Python \citep{van_rossum_python_2009} was used throughout this work, with
the following open source packages: scikit-image
...
\citep{clark_pillow_2015}, SunPy \citep{mumford_sunpy_2020}, OpenCV
\citep{bradski_opencv_2000} and Matplotlib \citep{hunter_matplotlib_2007}. All
code used in this work is available a \url{github.com/chemron/honours}
