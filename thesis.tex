...
indirect relationships in data. Due to the large quantity of data available,
increased accessibility to better hardware and the recent improvements in deep
learning algorithms, the past few years have seen deep learning become much more
commonplace as a tool for solar physics. Recently, \citet{Kim2019} used a deep
learning model known as a conditional Generative Adversarial Network (cGAN) to
learn a mapping between extreme ultraviolet (EUV) images and magnetograms. This
was then applied to EUV images taken by STEREO-A to generate partial-farside
magnetograms over the course of its mission. However, due to the aforementioned
drawbacks related to the STEREO mission, this provides limited utility for
monitoring the farside. In another example, \citet{felipe_improved_2019} used a
deep learning algorithm to detect farside active regions from seismic maps. This
improved on previous approaches and was able to predict active regions with
higher sensitivity. However, this work did not attempt to predict the farside
magnetic field, limiting the ability to predict extreme solar events. \\
...
\begin{figure}[t]
  ...
  } \caption{\subref{fig:hsm_power} The power spectrum of the Sun's p-mode
...
\begin{figure}[t]%
  ...
  \caption[]{\subref{fig:internal_rotation} The frequency of rotation of the
    ...
    \subref{fig:tacholine_rotation} The
...
\begin{figure}[t]%
  ...
  \subref{fig:butterfly diagram} A `Butterfly diagram' of the Sun, showing the
  evolution of sunspots throughout many solar cycles. \textit{Image courtesy of
  NASA}. \subref{fig:mag_butterfly} A Butterfly diagram of the Sun, this time
...
\begin{figure}[t]%
  ...
  \citet{sammis_dependence_2000}}. \subref{fig:flare_model} The CSHKP model for
...
\begin{figure}[t]%
  ...
  \caption[]{\subref{fig:skips} Depiction of p-modes propagating through the
    solar interior. \textit{Image courtesy of \citet{Lindsey2017}.}
    \subref{fig:egression} Disturbances on the solar surface (dark blue) can be
...
\begin{figure}[t]%
  ...
  \caption[]{
    ...
    \textit{Image courtesy of \citet{Lindsey2011}.}\subref{fig:phase_map} Images showing both a farside seismic map created using
...
\begin{figure}[t]%
  ...
  \caption[]{\subref{fig:hmi_p} The percentiles of the pixel values for
  the magnetogram data. \subref{fig:seismic_p} The percentiles of pixel
...
% Our model consists of a U-net style generator network
% \citep{ronneberger_u-net_2015}, and a fully convolutional discriminator
% network. 
...
} \caption{\subref{fig:6.11.11_STE2} A STEREO-A EUV image taken on October 31st
...
\begin{figure}[t]%
  ...
  \subfigure[]{%
    ...
    \includegraphics[height=2in]{17.11.11_STE.png}% 
  }%
  \subfigure[]{%
    \label{fig:6.11.11_def_MAG}%
    \includegraphics[height=2in]{17.11.11_MAG.png}% 
  }\\%
  \subfigure[]{%
    \label{fig:6.11.11_def_smap}%
    \includegraphics[height=2in]{17.11.11_smap.png}% 
  }%
  \subfigure[]{%
  \label{fig:6.11.11_default}%
  \includegraphics[height=2in]{17.11.11_def.png}% 
  }%
  \caption[]{Images relating to the first attempt of training the Seismic-GAN.
  \subref{fig:6.11.11_def_STE} A STEREO-A EUV image taken on November 11th 2011.
  \subref{fig:6.11.11_def_MAG} The corresponding magnetogram generated by the
  UV-GAN. \subref{fig:6.11.11_def_smap} The equivalent Seismic Map taken on
  November 17th 2011. \subref{fig:6.11.11_default} The corresponding magnetogram
...
\begin{figure}[t]%
  ...
  \includegraphics[height=2in]{17.11.11_kernal.png}%
  } \\
  \subfigure[]{%
  ...
  \caption[]{\subref{fig:6.11.11_ker_smap} A seismic map taken on November 17th
  2011. \subref{fig:6.11.11_kernal} The corresponding magnetogram generated by the Seismic-GAN, after training
  it on data with a smaller STEREO-A/seismic map time delay. \subref{fig:16.11.06_ker_smap} A seismic map
  taken 5 years later on the November 6th 2016. \subref{fig:16.11.06_kernal} The corresponding generated magnetogram.
...
\begin{figure}[t]%
  ...
  \includegraphics[height=2in]{17.11.11_fixed.png}% 
  }%
  \caption[]{\subref{fig:6.11.11_STE} A STEREO-A EUV image taken on November
  11th 2011. \subref{fig:6.11.11_MAG} The corresponding magnetogram generated
  by the UV-GAN, \subref{fig:6.11.11_smap} The equivalent Seismic Map taken
  on November 17th 2011. \subref{fig:6.11.11_SMAG} The corresponding
...
\begin{figure}[t]%
  ...
  \caption[]{\subref{fig:uv_mag_2011_11_15} A synthetic STEREO magnetogram
  generated by the UV-GAN, from a EUV image taken on the November 15th 2011.
  \subref{fig:smap_2011_11_15} The corresponding seismic map.
  \subref{fig:smag_2011_11_15} The synthetic magnetogram generated by the
...
\begin{figure}[t]%
  ...
  magnetogram \subref{fig:smag_2018peak} taken on October 23rd 2018, corresponding to a
...
% this comment is important: you need to link this back to what is not known about
% the sun, and ask yourself:

% "in the most optimistic setting possible, where your networks worked perfectly
% etc etc, what things could you have tested about the sun using your model? would
% the coefficients of the network be informative about understanding what kind of
% physcial models of the sun are suitable? would the long-term predictions (or
% historical predictions) be useful for understanding the maunder minimum? to know
% what dynamo models could be excluded?

% You *NEED* to write some paragraphs about this,.. even though we both agree it
% is "shooting for the moon a bit", because if you write nothing then the
% assessors will have to weight you down heavily on not linking back to the actual
% science. And that then gives you the opportunity to say what other extensions of
% this work could be useful. Your conclusions should link back to the science,
% too. 
...
% Farside magnetograms would provide an earlier warning of dangerous active
% regions before they rotate to the nearside. Furthermore, a better understanding
% of the solar dynamo woud not only lead to a better understanding of the Sun and
% Sun-like stars, but would allow for better long-term predictions of solar
% magnetic activity. This knowlege is critical in our ability to predict and
...
% understanding the emergence and evolution of active regions by permitting the
% tracking of active regions as they rotate around the Sun, in turn providing
% clues about the toroidal field that created them. Furthermore, interogatting
% this model could inform models of the Sun and lead to a better understanding of
% the interactions between p-modes and active regions.
