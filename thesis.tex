...
\begin{itemize}
  ...
  %TODO:
  shows a diagram of both GANs. This model was based on the one used by
  \citet{Kim2019} for a very similar purpose.
\end{itemize}
...
\begin{itemize}
  ...
  \item the specific architecture used in the is shown in Figure %TODO:
  
  
  
  Many `skip
  connections' join layers of the same size either side of the `U'. The model
  used in this work is shown in Figure %TODO:
  ...
  \item need image-to-image
  \item needs to maintain shape and large scale structures from original image
  \item needs to capture small scale variations, interactions between small
  sections of image
  \item Gan
  \item Generator
  \item U-Net
...
\subsection{Descriminator}

\begin{itemize}
  \item Descriminator
  \item 
  \item Why we used square root to artificially increase the saturation
\end{itemize}
