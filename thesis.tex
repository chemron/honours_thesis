...
peak solar flux measured at Earth is greater than \SI{1e-4}{W.m^{-4}}.}. There
is a clear bias between the two predicted fluxes, at the time of writing the
cause of this is unclear.  %TODO: fix, make plot of relative flux
Futhermore, It is clear that the magnetic flux predicted by the UV-GAN's
synthetic magnetograms had a higher variance than that of the true magnetograms.
Crucially however, the UV-GAN is able to produce magnetograms that
\begin{enumerate}
  \item accurately predict sharp changes in magnetic flux, including near X-class solar flares,
  \item reproduce the small scale variations in the magnetic flux, and
  \item reproduce the overall shape of the solar cycle.
\end{enumerate}
We now move on to the Seismic-GAN. 
...
\begin{itemize}
  \item Seismic magnetograms are limited by the UV magnetograms - since that is
  what they use to train
  \item doesn't have to produce true magnetgrams, just produce magnetograms
  similar to UV-magnetograms
  \item errors compound
  \item the bias in the uv magnetograms therefore carries over to the seismic magnetograms



  \item maybe talk about good stuff first
