...
the EUV images which had normalised pixel values between \SI{0}{DN} and
\SI{1}{DN} this artificial saturation was done by taking the square-root of the
pixel values. This ensured that the pixel values stayed between \SI{0}{DN} and
\SI{1}{DN}, while increasing the intensity of the under-represented pixels. For
the magentograms, which had pixel values between $\pm \SI{1}{DN}}$ this
artificial saturation took the form  
\begin{align}
...
\begin{itemize}
  ...
  \item the surface area of the sun corresponding to a given pixel changes with
  radius (i.e. pixels at the solar limb will correspond to a larger section of
  the Sun than pixels in the centre of the image).
