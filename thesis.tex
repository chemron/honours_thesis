...
\begin{itemize}
  \item dangers of the sun
  \item monitering the magnetic activity on the farside of the sun is vital
  \item NASA's STEREO mission moniters the farside of the Sun
  \item not capable of taking mangetograms  
  \item farside seismic maps
  \item Ganzalez Hernadez et al tried to calibrate the magnetic flux as a
  function of the seismic signature
  \item positive correlation was found between the seismic signal and magnetic
  flux, but the correlation found was poor.
  \item deep learning methods offer the ability to find complex and indirect relationships
  that may improve the model due to the large number of parameters 
  \item for example kim et al did the stereo thing
  \item however stereo is lacking
  \item felipe
  \item magnetic field
  \item what we did
\end{itemize}

The rich and complex solar magnetic field is the result of a self-regenerating
dynamo within the Sun. Flux ropes produced by a toroidal field may % what are flux ropes
rise and penetrate through the solar surface, forming an active region. These
active regions are home to extreme magnetic fields, with strengths typically
exceeding 1000 Gauss. The extreme conditions in active regions can lead to a
build up of magnetic energy. If the active region is unable to dissapate this
energy, this can lead to an energetic magnetic reconnection event causing an
explosive release of built up energy. Such eruptions can cause solar flares or
coronal mass ejections and pose a danger to earth.\\
\citep{Mewaldt2005, council_severe_2008}. \\



X-rays released by coronal mass ejections can heat the Earth's atmosphere
atmosphere causing drag on satellites and can pose a radiation hazard to
Astronauts. Coronal mass ejections that collide with the Earth create a
geomagnetic storm with the potnetial to overwhelm terrestrial electrical grids
causing blackouts. This has historically resulted in blackouts for example in
Quebec \citep{odenwald_day_2015}

magnetograms are a vital tool for predicting such events, and the related
magnetic flux ... etc are used as predictors.
While magnetograms are available for the nearside, these 
...
\citep{bobra_solar_2015}. predictors such as magnetic flux etc ...

However, these magnetograms are only available for the
...
NASA's Solar Terrestrial Relations Observatory (STEREO)\footnote{STEREO consists of
two spacecraft, STEREO ahead (A) and STEREO behind (B).} has provided some
monitoring of the solar farside throughout it's mission. However STEREO is not
capable of producing magnetograms, and is also only has a partial view of the solar
...
monitoring of the solar farside is elioseismic holography
\citep{lindsey_seismic_2000}. This is a helioseismic technique that maps the
farside by timing accoustic waves, known as p-modes, as they travel from the
...
however and provides limited information about the farside magnetic field. It is
possible that some indirect relationship between seismic seismic signature and
the farside magnetic field does exist, however to find such a relationship a
much more general model must be applied.\\


Deep learning models offer the ability to find complex and indirect
relationships due to a large number of parameters. Due to the large quantities
of data available, increased accessibilty to better hardware and the recent
improvements in deep learning, over the past few years deep learning has become
much more common as a tool for solar physics. \citet{Kim2019} used a deep
...
was than applied to EUV images taken by STEREO-A to generate partially farside
magnetograms over the course of it's mission. However due to the afforementioned
shortcomings of STEREO, this provides limited utility for monitoring the farside.\\

\citet{felipe_improved_2019} also used a deep neural network to detect farside
active regions from seismic maps. This improved on previous approaches and was
able to predict active regions with a higher sensitivity. However they do not
predict the level of magnetic activity on the farside, limiting the ability to
predict extreme solar events. \\


In this thesis we describe and implement a method to translate farside seismic
maps to farside magnetgorams. Our method consists of two steps. First, we train
...
generate magnetograms from farside seismic maps. In Chapter
\ref{chap:background}, we provide a background into the science behind our
method is provided along with a brief overview of the process. Chapter
\ref{chap:data} details how we prepare the data for deep learning. Chapter
\ref{chap:training} describes the architecture of our deep learning models and
describe the process of training them. In Chapter
\ref{chap:results_and_analysis} we analyse the performance of our model, and
discuss its ability to predict magnetic activity. In Chapter
\ref{chap:discussion} we discuss the our results as well as potential shortcomes
before concluding in Chapter \ref{chap:conclusion}.
...
\begin{figure}%
  ...
  \subref{fig:upsampling} Diagram of an upsampling layer, showing the input
